...
{
    public class LookuperRx09WithCancellationByTakeUntil : LookuperRxBase
    {
        private readonly TimeSpan _throttleDueTime;
        private readonly int _retryCount;
        private readonly TimeSpan _timeoutDueTime;

        public LookuperRx09WithCancellationByTakeUntil(ISearchEngine searchEngine)
            : this(searchEngine, throttleDueTime: TimeSpan.FromMilliseconds(500), retryCount: 3, timeoutDueTime: TimeSpan.FromSeconds(3))
        {
        }

        public LookuperRx09WithCancellationByTakeUntil(ISearchEngine searchEngine, TimeSpan throttleDueTime, int retryCount, TimeSpan timeoutDueTime)
            : base(searchEngine)
        {
